...
function safestEvent(view) {
  return view.actions.find(action => action.id.endsWith(':decline-event') || action.id.endsWith(':end-event')) ?? view.actions.find(action => action.id.endsWith(':reroll:team') || action.id.endsWith(':skill:true')) ?? view.actions.find(action => action.kind === 'reroll' && /^Do not /i.test(action.label)) ?? view.actions[0];
}
async function resolveEvents(view, limit = 30) {
  for (let count = 0; count < limit; count++) {
    const gameAction = view.actions.find(action => ['select', 'move', 'endTurn', 'endAction', 'selectBlock', 'block', 'pass', 'handOff', 'throwTeamMate', 'liftTeamMate'].includes(action.kind));
...
async function nextTurn(view) {
  view = await finishPlayerAction(await resolveEvents(view));
  const end = view.actions.find(a => a.kind === 'endTurn'); assert.ok(end, 'native end-turn option');
  return resolveEvents(await choose(pages[indexFor(end.actor)], end, view.revision + 1));
}
async function ballAttempt(view, kind) {
  if (!view.ball) return { view, submitted: false };
  const declaration = kind === 'pass' ? 'declarePass' : 'declareHandOff';
  const candidates = view.actions.filter(a => a.kind === declaration).map(action => ({ action, player: view.players.find(p => action.id.endsWith(`-${p.id}`)) })).filter(v => v.player?.x !== null);
  candidates.sort((a, b) => distance(a.player, view.ball) - distance(b.player, view.ball));
  if (!candidates.length || !view.ball) return { view, submitted: false };
  const chosen = candidates[0];
  view = await choose(pages[indexFor(chosen.action.actor)], chosen.action, view.revision + 1);
  for (let step = 0; step < 12; step++) {
    view = await resolveEvents(view);
    const player = view.players.find(p => p.id === chosen.player.id);
    if (view.activePlayerId !== player.id) return { view, submitted: false };
    const target = view.actions.find(a => a.kind === kind && (kind !== 'pass' || view.players.some(p => p.role === player.role && p.id !== player.id && a.id.endsWith(`pass-${p.x}-${p.y}`))));
    if (target) {
      view = await choose(pages[indexFor(target.actor)], target, view.revision + 1);
      return { view: await resolveEvents(view), submitted: true, action: target.kind };
    }
    const hasBall = view.ball && player.x === view.ball.x && player.y === view.ball.y;
    const destinations = hasBall ? view.players.filter(p => p.id !== player.id && p.role === player.role && p.x !== null) : [view.ball];
    if (!destinations.length || !destinations[0]) return { view, submitted: false };
    const moves = view.actions.filter(a => a.kind === 'move').map(action => {
      const match = /move-(\d+)-(\d+)$/.exec(action.id); return { action, x: Number(match[1]), y: Number(match[2]) };
    }).filter(move => destinations.some(target => distance(move, target) < distance(player, target)));
    moves.sort((a, b) => Math.min(...destinations.map(t => distance(a, t))) - Math.min(...destinations.map(t => distance(b, t))));
    if (!moves.length) return { view, submitted: false };
    view = await choose(pages[indexFor(moves[0].action.actor)], moves[0].action, view.revision + 1);
  }
  return { view, submitted: false };
}
...
try {
  ...
  const kickoff = view.actions.find(action => action.id.endsWith(view.actor === 'home' ? 'kick-16-7' : 'kick-9-7')) ?? view.actions[0];
  ...
  const submittedFamilies = [];
  for (const kind of ['handOff', 'pass']) {
    for (let attempt = 0; attempt < 6 && !submittedFamilies.includes(kind); attempt++) {
      const result = await ballAttempt(view, kind); view = result.view;
      if (result.submitted) submittedFamilies.push(kind);
      view = await nextTurn(view);
    }
    assert.ok(submittedFamilies.includes(kind), `live ${kind} must reach native resolution`);
  }
  // Both rosters contain the original catalog Ogre and Halfling, initially adjacent.
  for (let attempt = 0; attempt < 2 && !submittedFamilies.includes('throwTeamMate'); attempt++) {
    const declare = view.actions.find(a => a.kind === 'declareThrowTeamMate');
    if (declare) {
      view = await choose(pages[indexFor(declare.actor)], declare, view.revision + 1);
      for (let step = 0; step < 6 && !view.actions.some(a => a.kind === 'liftTeamMate'); step++) {
        const mate = view.players.find(p => p.role === declare.actor && p.slot === 5 && p.x !== null);
        if (!mate) break;
        const moves = view.actions.filter(a => a.kind === 'move').map(action => {
          const match = /move-(\d+)-(\d+)$/.exec(action.id); return { action, x: Number(match[1]), y: Number(match[2]) };
        }).sort((a, b) => distance(a, mate) - distance(b, mate));
        if (!moves.length) break;
        view = await resolveEvents(await choose(pages[indexFor(moves[0].action.actor)], moves[0].action, view.revision + 1));
      }
      const lift = view.actions.find(a => a.kind === 'liftTeamMate');
      if (lift) {
        view = await resolveEvents(await choose(pages[indexFor(lift.actor)], lift, view.revision + 1));
        const target = view.actions.find(a => a.kind === 'throwTeamMate');
        if (target) { view = await resolveEvents(await choose(pages[indexFor(target.actor)], target, view.revision + 1)); submittedFamilies.push('throwTeamMate'); }
      }
    }
    view = await nextTurn(view);
  }
  assert.ok(submittedFamilies.includes('throwTeamMate'));
  ...
  await writeFile(resolve(out, 'core-turn-summary.json'), JSON.stringify({ passed: true, browser: await browser.version(), matchId, kickoffAction: kickoff.kind, pendingPrompt: pending.kind, completedTurns, moves, submittedFamilies, pendingReconnect: true, exactRetry: true, wrongRoleRejected: true, staleRejected: true, finalRevision: home.revision, errors }, null, 2));
  console.log(`PASS ${matchId}: kickoff, block-die recovery, exact retry, and ${moves} safe moves across ${completedTurns} engine turns.`);
} catch (failure) {
  await writeFile(resolve(out, 'failure.json'), JSON.stringify({ message: String(failure), traces: await Promise.all(pages.map(page => page.evaluate(() => ({ incoming: window.__m3b?.incoming, outgoing: window.__m3b?.outgoing })))) }, null, 2));
  throw failure;
} finally {
  await browser.close();
}
