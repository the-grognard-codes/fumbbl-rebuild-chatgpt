...
const contextA = await browser.newContext(); const contextB = await browser.newContext(); const page = await contextA.newPage(); const observer = await contextB.newPage();
try {
  const install = async item => item.addInitScript(({ snapshot }) => { class MockSocket { static OPEN = 1; readyState = 1; outgoing = []; constructor() { window.resultSocket = this; queueMicrotask(() => this.onopen?.()); } send(raw) { const request = JSON.parse(raw); this.outgoing.push(request); if (request.type === 'join') queueMicrotask(() => { this.onmessage?.({ data: JSON.stringify({ version: 1, type: 'result', requestId: request.requestId, status: 'accepted', code: 'JOINED', revision: 0, duplicate: false }) }); this.onmessage?.({ data: JSON.stringify({ ...snapshot, actor: sessionStorage.getItem('subject') ?? 'home' }) }); }); } close() { this.readyState = 3; this.onclose?.(); } } window.WebSocket = MockSocket; }, { snapshot: snapshot('home') });
  await install(page); await install(observer);
